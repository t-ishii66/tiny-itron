...
| ROW_TASK3 | 6 | [CPU0] Task3 カウント + LOCK/BUSY/---- |
| ROW_SHARED | 7 | 共有カウンタ (sem 1 保護) |
| ROW_TASK2 | 8 | [CPU1] Task2 カウント + LOCK/BUSY/---- |
| ROW_KBD | 9 | [CPU1] Task4 キーボード入力エコー |
| ROW_PFTEST | 11 | ページフォルトテスト結果 |
...
**注意点:** 実際の遅延時間は CPU クロックと QEMU のエミュレーション速度に依存する。RTOS のタイマーサービス (dly_tsk) がスタブのみのため、代替手段として使用。
