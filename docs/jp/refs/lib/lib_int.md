...
**戻り値:** `TRUE` -- CS != SEL_K32_C (ユーザモード / Ring 3)、`FALSE` -- CS == SEL_K32_C (カーネルモード / Ring 0)

**処理内容:** **syscall を使用しない。** `get_cs()` でアセンブリにより現在の CS セレクタを取得し、`SEL_K32_C` (0x20: カーネルコードセグメント) と比較する。CS が `SEL_K32_C` であれば `FALSE`、それ以外なら `TRUE` を返す。

**特記事項:** ITRON 仕様では `sns_ctx` は非タスクコンテキスト (割り込みハンドラ内) で TRUE を返す関数だが、本実装は CS セレクタの Ring レベルで判定しており、ユーザモード (Ring 3) で呼ぶと常に TRUE、カーネルモード (Ring 0) で呼ぶと常に FALSE を返す。ユーザタスクは Ring 3 で動作するため、タスクコンテキストから呼んでも TRUE が返る点に注意。コメントアウトされた `syscall(-TFN_SNS_CTX)` がコード中に残っている。
