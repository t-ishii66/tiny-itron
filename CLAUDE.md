...
- **言語**:
  - ソースコード中のコメント: **英語のみ**
  - ドキュメント: **日本語が原本**、その後英語に翻訳。必ずこの順番
  - ディレクトリ構成: `docs/ja/` (日本語原本)、`docs/en/` (英語翻訳)
  - 翻訳時は内容の同期を保つ。日本語を変更したら英語も更新する
- **正確さの原則**:
...
```bash
# ビルド
make              # トップレベル Makefile が kernel → lib → i386 の順にビルド
make clean        # 全体クリーン
